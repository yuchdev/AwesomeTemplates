...
- Tests are flat under `tests/`; `tests/flake8_lint/` is the separate flake8
  plugin suite. Shared fixtures live in `tests/conftest.py`.
- Coverage baseline:
  `uv run pytest -q --cov=awesome_templates --cov-report=term-missing`.
...
2. **Integration tests** (in the flat `tests/` suite) - exercise wiring with
   mocked externals and verify full pipeline stages and event order.
