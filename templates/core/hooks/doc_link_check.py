...
a real heading slug in the target file - entirely inline, no external
`scripts/` dependency (the link/anchor logic lives in `_common.py`, shared
with `doc_registry.py` and `linkify_doc_mentions.py`).
...
(used by the `/link-check` skill). With no paths, scans the whole corpus.
