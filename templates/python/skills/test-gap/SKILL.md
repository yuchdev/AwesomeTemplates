...
1. Establish current coverage:
   `uv run pytest tests/unit/ -q --cov={{PROJECT_PACKAGE}} --cov-report=term-missing`
   (scope with `--cov={{PROJECT_PACKAGE}}.<subpkg>` when a path is given).
2. Spawn the **`testing-expert`** agent with the coverage output and the target path. Ask it
   ...
   (blast-radius criteria + {{PROJECT_NAME}} module examples per tier):
