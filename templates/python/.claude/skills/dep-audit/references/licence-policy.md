# Licence compatibility policy

Deterministic ruling for the licence step of `/dep-audit`. The tiers below
...
distribution, may reasonably downgrade some ⛔ Block entries to ⚠️ Flag or
✅ Allow (AGPL's network-copyleft trigger is the one exception worth keeping
strict even for SaaS-only, no-distribution products). Classify every direct
dependency's licence into one of the three tiers below and report accordingly.
...
- **SSPL**, **BUSL/Commons-Clause**, and other source-available/non-OSI licences.
- **No licence / UNKNOWN / "proprietary"** on a third-party package — treat as
...
2. Identify each licence: prefer `pip-licenses`; fall back to the package's
...
Report `UNKNOWN` licences explicitly — a silent omission reads as "all clear"
