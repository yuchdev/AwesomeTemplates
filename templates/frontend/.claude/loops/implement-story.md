...
and task queue do **not** change within a run - re-parsing the ~500-line `plan.md` and
...
```json
{
  ...
  "story_folder": "docs/roadmap/0001-working-implementation/01.0-hello-world-page",
  "task_heading": "## Tasks",
  "task_queue": [
...
task may run concurrently; units with stated ordering dependencies run sequentially. This
...
|---------------------|------------------------------------------|------------------------------|
| `Frontend Expert`  | `frontend-expert`                           | task implementation      |
| `Testing Expert`   | `testing-expert` then `frontend-expert`     | QA then fix regressions     |
...
- The task spec **path** (`{story_folder}/{NN}-{task-slug}.md`) - the agent reads it
  itself. Add a one-line scope note, not the spec text.
- The cross-cutting rules in `@docs/dev/frontend_coding_standard.md` - JSDoc comment coverage,
  semantic/accessible markup, no swallowed `catch {}`, unit + mocked-integration tests.
...
Run **only this task's** verification, not the whole suite: scope the test run to the
task's own test target(s) (e.g. `npx vitest run tests/unit/task-foo.test.js`, or
`node --test tests/unit/task-foo.test.js`), plus the project's lint task (`npm run lint`,
or the narrower check the repo actually uses - see `frontend-expert.md`). Do **not** pipe full test output into context - a green
run is one summary line; on failure, tail only the failing assertions. The full suite runs
once at story completion (Step 7), and the Stop hook gates the session end regardless.

On failure:

- Diagnose, delegate the fix to `frontend-expert`, re-run. Repeat until green or until the
...
- **FAIL** → delegate the blocking gaps back to `frontend-expert` (max 1 retry), re-run
...
testing-expert's own per-task coverage note (Step 3) is a substitute for a repo-wide
dependency audit. -->
...
> enhancement tasks (e.g. a caching optimisation) once the core feature is delivered and
...
- **Story complete** → this is the **one** place the whole suite runs: execute
  the project's full test command, e.g. `npm test` (plus its coverage task, if it has one) once (summary line
...
> Cache caveat: a heavy `frontend-expert` step that runs longer than ~5 min blows the TTL
...
tasks, a security-sensitive subsystem that needs extra review before coding starts, an
artifact category (generated code, vendored fixtures, build output) that should skip the
usual lint/style gate, or a dependency that must land before a task can pass. Keep each
...
roster. For one-off spec-compliance audits of an already-built story, invoke the
`/verify-task` skill directly against each task spec - no separate audit loop exists
in this preset.
