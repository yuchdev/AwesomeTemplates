...
1. **Resolve the spec path** from `$ARGUMENTS`.
   ...
   - If the file does not exist, error: "Spec not found: <path>. Pass the path relative to
     docs/roadmap/ without the .md extension, e.g. `0001-working-implementation/01.0-hello-world-page/01-page-skeleton`."
...
4. **Return** the agent's compliance report verbatim.
   ...
   - FAIL → list blocking gaps; instruct caller to return to `frontend-expert` with the list.
