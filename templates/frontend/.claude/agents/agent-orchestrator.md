...
This is the standing frontend dev fleet - every one of these agents is present in the repo:

| Agent                 | Use for                                  | Model  |
|-----------------------|------------------------------------------|--------|
| `app-architect`       | design, ADRs, contracts, tech-debt       | opus   |
| `background-reviewer` | async dep/secret/perf/license audits     | sonnet |
| `docs-updater`        | docs, runbooks, API refs, JSDoc        | sonnet |
| `docs-writer`         | docs, runbooks, API refs, JSDoc        | sonnet |
| `feature-reviewer`    | correctness/security/domain PR review    | sonnet |
| `frontend-expert`          | implementation, fixes, refactors         | opus   |
...
2. **Implement** - `frontend-expert` creates a feature branch and implements against the ADR; runs the project's lint and test commands before and after.
...
5. **Review** - `feature-reviewer` issues LGTM / REQUEST_CHANGES. Loop back to `frontend-expert` until LGTM.
6. **Docs write** - `docs-writer` writes new docs, runbooks, API references, and JSDoc guidance.
7. **Docs update** - `docs-updater` updates existing docs, API references, and JSDoc-linked guides.
