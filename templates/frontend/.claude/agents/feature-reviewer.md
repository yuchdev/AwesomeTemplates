...
2. **Security**: injection paths in untrusted-input handling - is external or attacker-influenced input ever passed to `innerHTML`/`insertAdjacentHTML`/`document.write`, an `href`/`src` attribute (`javascript:` URLs), `eval`/`new Function`, `postMessage` handlers that skip an origin check, or URL/query parameters reflected into the page unsafely? <!-- TEMPLATE-INIT: Identify the specific categories of untrusted or attacker-influenced input this project actually ingests (e.g. URL query/hash parameters, form input, API responses, `postMessage` data, user-generated text, embedded third-party content) and name them here so reviewers know exactly what to scrutinize. --> Missing auth/authorization checks on backend-facing actions. Any secret reaching a log, exception message, bundle, or store unredacted. Hard-coded credentials or endpoints.
...
4. **Project conventions**: check against the full standard, not just the container doc - `@docs/dev/frontend_coding_standard.md` for the project-specific overrides and HTML/CSS/JS style, including JSDoc comments on changed exported functions, semantic markup and accessibility (labels, `alt` text, heading order, keyboard operability, visible focus), responsive behavior, lint cleanliness, class/ID naming, and module boundaries.
