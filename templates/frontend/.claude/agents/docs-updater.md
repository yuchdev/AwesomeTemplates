...
- **JSDoc comments and public API docs**: ensure every exported function, class, module, and agent interface touched by a change carries a JSDoc comment matching project style. Flag any public symbol that lacks it.
...
Docs here are linked from each other **and from code** (comments and JSDoc comments may say `See docs/specs/…md §X`). Treat every doc as a node in a reference graph: editing a heading, path, or section has a blast radius.

- Before renaming/moving/splitting a file or section - or rewording a heading or a paragraph other docs summarise - run **`/doc-xref <target>`** to enumerate every inbound reference (in `docs/**`, repo-root `*.md`, `.claude/**`, and `src/**` / `test/**` comments and JSDoc comments) and update them in the *same* change.
...
- [ ] No orphaned references to a moved/renamed/split file or heading remain - re-run `/doc-xref` on anything you renamed, **including code comments and JSDoc comments**.
