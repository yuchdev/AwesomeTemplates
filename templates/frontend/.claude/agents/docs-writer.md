...
- Add or request JSDoc comments on every exported function, class, module, or agent interface you write about. Flag any public symbol that lacks it.
...
Docs in this repo are linked from each other **and from code** (comments and JSDoc comments may reference `docs/specs/…md §X`). Every doc you create becomes a node in a reference graph - name it and structure its headings carefully from the start.
...
- After creating a new file, run **`/doc-xref <topic>`** to discover whether existing docs or comments/JSDoc comments already point to an assumed path or anchor for it - update those references in the same change.
...
3. **Delegate for deep detail** - Before writing sections that depend on non-obvious implementation knowledge, ask the right agent:
   ...
   |-------------------------------------------|------------------|-------------------------------------------------|
   | Internal implementation details needed    | `frontend-expert`    | "Describe how X works so I can document it."    |
...
- [ ] Every public symbol covered by the new doc carries a JSDoc comment; flag any that do not.
