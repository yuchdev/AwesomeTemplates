...
This loop takes **one task** (e.g. "AWS Bedrock Backend", task `3.2`) and drives it to
...
- `0001/3.2` or `0001 3.2` - explicit milestone + task number.
- `3.2` - task number only; resolve the milestone by scanning every
  `docs/roadmap/{NNNN}-*/plan.md` `## Tasks` table for that number.
- `"AWS Bedrock Backend"` - task name; matched (case-insensitive, substring) against the
...
```json
{
  ...
  "task_number": "3.2",
  "task_name": "AWS Bedrock Backend",
  "task_folder": "docs/roadmap/0001-generic-implementation/03.2-aws-bedrock-backend",
  "subtask_heading": "## Subtasks",
  "subtask_queue": [
    {"nn": "01", "slug": "configuration-model", "status": "complete"},
