...
1. Establish current coverage:
   `uv run pytest tests/unit/ -q --cov=awesome_templates --cov-report=term-missing`
   (scope with `--cov=awesome_templates.<subpkg>` when a path is given).
2. Spawn the **`testing-expert`** agent with the coverage output and the target path. Ask it
   ...
   (blast-radius criteria + Awesome Templates module examples per tier):
