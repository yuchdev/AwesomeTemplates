...
Skip the gate for a known-WIP session by exporting ``AWESOME_TEMPLATES_SKIP_TESTS=1``.
"""
...
def main() -> None:
    ...
    if event.get("stop_hook_active"):
        allow()
    if os.environ.get("AWESOME_TEMPLATES_SKIP_TESTS") == "1":
        append_log("tests.log", "SKIPPED (AWESOME_TEMPLATES_SKIP_TESTS=1)")
        allow()

    steps = [
        ("ruff --fix", ["uv", "run", "ruff", "check", ".", "--fix"]),
        ("ruff check", ["uv", "run", "ruff", "check", "."]),
        ("pytest+cov", ["uv", "run", "pytest", "-q", "--cov=awesome_templates", "--cov-report=term-missing"]),
    ]
    ...
    for label, cmd in steps:
        ...
        if proc.returncode != 0:
            ...
            block(
                f"Stop blocked by Awesome Templates run-tests: `{' '.join(cmd)}` failed.\n"
                f"{_tail(proc)}\n"
                ...
                "guessing. Export AWESOME_TEMPLATES_SKIP_TESTS=1 only if this is intentional WIP."
            )
