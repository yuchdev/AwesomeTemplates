...
- The cross-cutting rules in `@docs/dev/python_coding_standard.md` - full annotations, ruff,
  ...
  Also name these sections of this project's own guidance, which the shipped coding
  standard does not cover: the root `CLAUDE.md`'s **"Executable tooling: `.claude/hooks/`
  vs `scripts/` (two tiers, one rule each)"** (which tier a new executable belongs in is
  decided by what *triggers* it; a hook may never import from `scripts/`; the enumerated
  "what is not allowed" list is a record of bugs that have actually shipped here), its
  **"Ruff config note"** (the root `ruff.toml` - not `pyproject.toml`'s `[tool.ruff]`
  block - is what `ruff check` applies, and the two deliberately disagree), and its
  **"The template catalog (`templates/`)"** section (the `python` and `java` presets are
  independent self-contained copies, so near-identical files such as each preset's
  `_common.py` must not be "deduplicated" into a shared location). For any change under
  `src/awesome_templates/`, add `src/awesome_templates/CLAUDE.md`'s **"Module
  map"** - it is the per-module authority and carries the hard boundary that `ai/client.py`
  is the only module permitted to import `anthropic`, and only lazily inside functions. The
  root `AGENTS.md` is a near-duplicate of an older revision of `CLAUDE.md`; prefer
  `CLAUDE.md` and treat `AGENTS.md` as stale where they differ. If the milestone's
  `plan.md` carries its own cross-cutting guidelines section, name it for the agent to read.
- The specific anchor in `docs/roadmap/{NNNN}-{milestone-slug}/` the subtask references -
  the section, not the whole doc. This project keeps its detailed designs there rather than
  in `docs/adr/` or `docs/specs/`, neither of which exists at this repo's root. Two
  conventions are in use and both are anchored by `##` headings, not numbered sections:
  milestone `0001-ai-assisted-generation/` is a set of numbered design documents named
  `{NN}.{Title_With_Underscores}.md` (e.g.
  `03.Agentic_marker_research.md#root-cause`), while `0002-api-based-marker-research/` uses
  the `plan.md` + `status.md` pair with tasks numbered `01`-`05` in `plan.md`'s `## Tasks`
  table. Cite `path#heading-slug`, never the file alone.
...
| Condition                                                | Skill                                                                                                                                |
|----------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------|
| Always                                                   | `/test-gap src/awesome_templates/<subtask-module>/` - report coverage delta; if < 85 % delegate missing tests to `testing-expert`. |
| `pyproject.toml` or any `requirements*.txt` changed      | `/dep-audit`                                                                                                                         |
| Any file under an API route, auth, or middleware path changed, or anywhere handling credentials/tokens | `/secret-scan src/awesome_templates/<changed-module>/` |
...
- **Task complete** → this is the **one** place the whole suite runs: execute
  `uv run pytest tests/unit/ -q --cov=awesome_templates` once (summary line only), then run
